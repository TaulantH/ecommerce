...
// check session
    ...
    // define variables
    $name = $price = $details = $category = $picture = $availability =  ""; $display = 1;
    ...
    if (isset($_POST['create'])) {
        ...
        if (!$error) {
            ...
            $sql = "INSERT INTO `products`(`name`, `price`, `details`, `picture`, `availability`, `fk_category`,`display`) VALUES (?, ?, ?, ?, ?, ?,?)";
            
            $stmt = mysqli_prepare($connect, $sql);

            if ($stmt) {
                
                mysqli_stmt_bind_param($stmt, "sdsssii", $name, $price, $details, $picture[0], $availability, $category,$display);
